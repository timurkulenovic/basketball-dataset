...
games_data = []
if not os.path.exists('../../../data/euroleague'):
    os.makedirs('../../../data/euroleague')

req_main = requests.get(f'{BASE_URL}/main/results')
bs_main = bs(req_main.text, "html.parser")
season_sel = bs_main.find_all("div", {"class": "styled-select"})[0]

for season_opt in season_sel.find("select").find_all("option")[:-2]:
    ...
    for season_part_opt in season_part_sel.find("select").find_all("option"):
        ...
        for round_opt in round_sel.find("select").find_all("option"):
            ...
            for game in games:
                ...
                # Box score Home team
                h_stats_td = bs_game.find("div", {"class": "LocalClubStatsContainer"}) \
                                 .find("table").find("tfoot").find("tr", {"class": "TotalFooter"}).find_all("td")[4:]
                h_stats = [td.text.split("/") if "/" in td.text else td.text for td in h_stats_td]
                h_stats = h_stats[0] + h_stats[1] + h_stats[2] + h_stats[3:]

                # Box score Away team
                a_stats_td = bs_game.find("div", {"class": "RoadClubStatsContainer"}) \
                                 .find("table").find("tfoot").find("tr", {"class": "TotalFooter"}).find_all("td")[4:]
    ...
    df_euroleague.to_csv("../../data/euroleague/euroleague_games.csv", index=False)
