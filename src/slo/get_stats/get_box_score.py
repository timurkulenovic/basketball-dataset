...
def get_box_score(bs_game, game_data, BASE_URL):
    ...
    for row in rows:
        ...
        if three_point == "":
            three_points_att, three_points_made = 0, 0
        else:
            three_points_made, three_points_att = three_point.split(delimiter)
            three_points_att = 0 if three_points_att == "" else int(three_points_att)
            three_points_made = 0 if three_points_made == "" else int(three_points_made)
